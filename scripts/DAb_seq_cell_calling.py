...
@HZ 09/2022: made some amendments
...
def call(
    ...
    '''
    (Ben:) call cells using selected method. returns list of valid cell barcodes.
    if threshold is on, further refine to cells with 'min_coverage' in 'min_fraction' intervals.
    (HZ:) added some small tweaks which include:
    ...
    ****
    ...
    params:
        ...
        experiment_name: str
            name of sample, used for output naming
        min_reads: int
            minimum number of reads per cell
        method: str
            method to use for cell calling. options are: 'second_derivative', 'simple_minimum', 'hard_cutoff'
        threshold: bool
            whether to apply thresholding to cell calling
    ...
    all_tsv = Path(input_tsv)
    # all_tsv = output_dir + experiment_name + '.all.tsv'  # alignment counts for all barcodes (previously saved)
    all_df = pd.read_csv(all_tsv, sep='\t', header=0, index_col=0)
    
    # extract barcodes and read totals
    barcodes = list(all_df.index)
    reads_per_cell = all_df.sum(axis=1).astype(int).tolist()
    reads_per_cell, barcodes = (list(t) for t in zip(*sorted(zip(reads_per_cell, barcodes), reverse=True)))
    hard_filtered_df = all_df.loc[
        all_df.sum(axis=1) > min_total_reads_per_cell
        ]
    logger.info(f' Hard filtered {len(all_df.index) - len(hard_filtered_df.index)} barcodes with total reads per cell < {min_total_reads_per_cell}')
    hard_filtered_df.to_csv(
        output_dir / f'{experiment_name}.total_rpc-{min_total_reads_per_cell}.csv',
        index = True, header=True
    )
    logger.info(' Saved hard filtered barcodes to file')
    ...
    if method == 'second_derivative':
        ...
        rpc_thresh = [x for x in reads_per_cell if x >= min_total_reads_per_cell] # @HZ minimum total reads per cell is set as 100
        ...
        try:
            ...
            logger.info(f' The max peak is found at {n_cells} cells')
            cell_barcodes = barcodes[:n_cells]

        except ValueError:
            logger.warning(" With second derivative method, no peak found; falling back on using simple minimum!")
    ...
    # cell_tsv = output_dir / f'{experiment_name}.cells.tsv'
    cell_df.to_csv(
        output_dir / f'{experiment_name}.cells.tsv', 
        sep='\t'
        )

    # generate knee plot and amplicon boxplots
    kneeplot_path = output_dir / f'{experiment_name}.kneeplot.png'
    boxplot_path = output_dir / f'{experiment_name}.amplicon_boxplot.png'
    ...
    knee_plot(all_df, cell_df, str(kneeplot_path), note)
    amplicon_boxplot(cell_df, str(boxplot_path))

    # @HZ write a cell barcode to numerical index file
    bars_map_df = pd.DataFrame(valid_cells, columns = ['cell_barcode'])
    bars_map_df['num_index'] = np.arange(len(valid_cells)) + 1
    bars_map_df['num_index'] = 'cell_' + bars_map_df['num_index'].astype(str)
    bars_map_df.set_index('num_index', inplace = True)
    bars_map_df.to_csv(
        output_dir / f'{experiment_name}.barcode_map.txt', 
        sep='\t', 
        index=True
        )
...
def knee_plot(all_df, cell_df, output_file, note=''):
    ...
    logger.info(f'Knee plot saved to {output_file}')
...
def amplicon_boxplot(cell_df, output_file):
    ...
    plt.tight_layout()
    plt.savefig(output_file, dpi=300)
    logger.info(f'Amplicon boxplot saved to {output_file}')
